...
tf = 1.2; % Thoiwf gian chuyeenr ddoongj
...
% Quyx ddaoj Kane
for i=1:length(t)
x(i) = xo + (xf-xo)*(t(i)-to)/(tf-to)-((xf-xo)/(2*pi))*sin(2*pi*(t(i)-to)/(tf-to));
end
...
yd = differ(y,step); 
zd = differ(z,step); 
xdd = differ(xd,step); %DDaoj hamf caaps hai
ydd = differ(yd,step);
zdd = differ(zd,step);
...
% Giair he? vi phaan cuar banf mays ddoongj ddeer timf nhaan tuwr lagrange
...
% Thay cacs gias trij tinhs momeny khopws chur ddoongj
tau1 = 0*t;
tau2 = 0*t; 
tau3 = 0*t;
for i=1:length(t)
...
figure(1); % DDof thij moment ngaaux lucwj
